    ...
    <div>
      ...
      <AdminTableShell>
        <table className="w-full text-sm table-shell border-0 rounded-none">
          <thead>
            <tr><th>ID</th><th>{t('common.booking')}</th><th>{t('common.amount')}</th><th>{t('admin.session')}</th><th>{t('common.status')}</th><th>{t('common.date')}</th></tr>
          </thead>
          <tbody className="text-roma-muted">
            {payments.map((p) => (
              <tr key={p.id}>
                ...
                <td className="font-semibold text-white">{parseFloat(p.amount).toFixed(2)} {t('common.omr')}</td>
                <td className="text-xs font-mono text-roma-subtle">{p.thawani_session_id?.slice(0, 16)}…</td>
